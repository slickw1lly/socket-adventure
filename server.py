...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        """
        return [
            'Red Room', 'Blue Room', 'Green Room', 'Purple Room'
        ][room_number]
    ...
    def get_input(self):
        ...
        received = b''
        while b'\n' not in received:
            received += self.client_connection.recv(self.buffer_size)
    ...
    def move(self, argument):
        ...
        * "west"
        ...
        if self.room == 0:
            ...
            if direction == "east":
                self.room = 2
        if self.room == 1 and direction == "east":
            self.room = 0
        if self.room == 2 and direction == "west":
            self.room = 0
        if self.room == 3 and direction == "south":
            self.room = 0
